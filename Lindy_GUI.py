...
def calculate_age(born):
    ...
    try:
        ...
        print(born)
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Проверьте правильность заполнения колонки Дата_рождения_получателя')
        quit()
    except ValueError:
        print(f' Ошибка при подсчете текущего возраста ячейки {born}')
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Пустая ячейка с датой или некорректная запись в колонке Дата_рождения_получателя !!!')
        quit()
    except:
        print(f' Ошибка при подсчете текущего возраста ячейки {born}')
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Не удалось обработать данные в колонке Дата_рождения_получателя \nПроверьте файл!')
        quit()
...
def create_doc_convert_date(cell,*args):
    ...
    try:
        ...
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', f'Проверьте правильность заполнения ячеек с датой в колонке\n{args[0]}!!!')
    except TypeError:
        print(cell)
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Проверьте правильность заполнения ячеек с датой в колонке\n{args[0]}!!!')
        quit()
...
def convert_date(cell,*args):
    ...
    try:
        ...
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', f'Пустая ячейка с датой или некорректная запись в колонке\n{args[0]}!!!')
        quit()
...
def extract_date_begin_course(cell:str):
    ...
    try:
        ...
        print(cell)
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Проверьте правильность заполнения ячейки \nПериод_обучения_в_формате_с_дата_начала_по_дата_окончания!!!'
...
def create_initials(fio):
    ...
    if type(fio) == float:
        print(fio)
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Проверьте правильность написания ФИО в столбце ФИО_именительный')
        quit()
...
def generate_docs_dpo():
    ...
    try:
        ...
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', f'Выберите шаблон,файл с данными и папку куда будут генерироваться файлы')
    else:
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', 'Создание документов успешно завершено!')
    ...
    try:
        ...
        if status_rb_type_doc == 0:
            ...
            try:
                ...
                messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Колонка с ФИО должна называться ФИО_именительный')
                quit()
            except:
                messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11',
            ...
            try:
                ...
                messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Закройте открытый файл Word')
                quit()
            except:
                messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11',
                                     'Проверьте содержимое шаблона\nНе допускаются любые символы кроме _ в словах внутри фигурных скобок\nСлова должны могут быть разделены нижним подчеркиванием')
...
def generate_docs_other():
    """
    Функция для создания документов из произвольных таблиц(т.е. отличающихся от структуры базы данных Создание документов и отчетов ЦОПП версия 1.11)
    :return:
    """
    try:
        # Получаем название для создаваемых документов
...
def create_report_one_pk():
    ...
    try:
        ...
        if df_dpo.shape[0] == 0:
            messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Лист с данными ПК не заполнен!')
        ...
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', 'Создание части отчета 1-ПК\nЗавершено!')
...
def create_report_one_po():
    ...
    try:
        ...
        if df_po.shape[0] == 0:
            messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11','Лист с данными ПО не заполнен!')
        ...
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', 'Создание части отчета 1-ПО\nЗавершено!')
...
def create_report_svod():
    ...
    try:
        ...
        messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Проверьте названия листов! Должно быть ДПО и ПО')
    # except KeyError:
    #     messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11', 'Названия колонок не совпадают')
    # except:
    #     messagebox.showerror('Создание документов и отчетов ЦОПП версия 1.11',
    #                          'Возникла ошибка')
    else:
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', 'Сводный отчет успешно создан!')
...
def create_general_table():
    ...
    #                          'Возникла ошибка,проверьте шаблон таблицы\nДобавляемы файлы должны иметь одинаковую структуру с шаблоном таблицы')
        ...
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11', 'Общая таблица успешно создана!')
...
def calculate_data():
    ...
    if count_errors != 0:
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11',
...
def create_report_indicator():
    ...
    try:
        ...
        messagebox.showinfo('Создание документов и отчетов ЦОПП версия 1.11','Создание отчета по индикаторам и госзаданию завершено')


if __name__ == '__main__':
    window = Tk()
    window.title('Создание документов и отчетов ЦОПП версия 1.11')
